...
export const getLists = async (boardId, dispatch) => {
  ...
  try {
    const res = await api.get(listRoute + "/" + boardId);
...
export const activityUpdate = async (boardId, dispatch) => {
  ...
  try {
    const res = await api.get(boardRoute + "/" + boardId + "/activity");
...
export const createList = async (title, boardId, dispatch) => {
  dispatch(setLoading(true));
  try {
    const res = await api.post(listRoute + "/create", {
      title: title,
      boardId: boardId,
    });
...
export const DeleteList = async (listId, boardId, dispatch) => {
  dispatch(setLoading(true));
  try {
    await api.delete(listRoute + "/" + boardId + "/" + listId);
...
export const listTitleUpdate = async (listId, boardId, title, dispatch) => {
  try {
    ...
    await api.put(listRoute + "/" + boardId + "/" + listId + "/update-title", {
      title: title,
    });
  } catch (error) {
    dispatch(
...
export const boardDescriptionUpdate = async (
  ...
  try {
    ...
    await api.put(`${boardRoute}/${boardId}/update-board-description`, {
      description,
    });
  } catch (error) {
    dispatch(
...
export const boardBackgroundUpdate = async (
  ...
  try {
    ...
    await api.put(`${boardRoute}/${boardId}/update-background`, {
      background,
      isImage,
    });
  } catch (error) {
...
export const boardMemberAdd = async (boardId, members, dispatch) => {
  try {
    const result = await api.post(`${boardRoute}/${boardId}/add-member`, {
      members,
    });
